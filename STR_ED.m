        ...
        if size(gtRes2,1)>1
            for i=1:size(gtRes2,1)
                ...
                if size(predwords,2)==0
                    fprintf('TOTAL_EDIT_DISTANCE =  %d, EDIT_DISTANCE_RATIO = 1\n', size(img_gt,1)); 
                else
                    if(i==1)
                    temp_predwords=zeros(length(predwords),1);
                    end
                    if ~isempty(bboxes)
                    temp_predwords(i,:)=length(predwords{i});
            ...
            for ed_i=1:size(img_gt,1)
                for ed_j=1:length(predwords)
                    assignment_mat(ed_i,ed_j) = EditDistance(lower(cell2mat(img_gt(ed_i,5))),cell2mat(predwords(ed_j)));
            ...
            for search_dist=0: max_dist
                for ii=1:size(assignment_mat,1)
%                     if ii>=size(assignment_mat,1)
%                         ii=size(assignment_mat,1);
%                     end
...
%                 cout << " GT word \"" << words_gt[j] << "\" no match found" << endl;
                fprintf('GT word \"%s\" no match found\n', cell2mat(img_gt(i2,5))); 
...
%                     //cout << " Detection word \"" << words_detection[j] << "\" no match found" << endl;
                    fprintf('DT word \"%s\" no match found\n', cell2mat(words_detection_matched(j2))); 
                    total_edit_distance=total_edit_distance+ length(cell2mat(words_detection_matched(j2)));
                end
               end
    ...
    if size(dtRes,1)>1
        for i=1:size(dtRes,1)
            ...
            if numbbox3~=0 &&~isempty(wbboxes)
                ...
                %
                numbbox4 = size(wbboxes,1);
                pred_taken = zeros(numbbox4,1);
                
                
                for bidx = numbbox4:numbbox4
                    if pred_taken(bidx)==0
                        ...
                        for worse_idx = (bidx+1):numbbox4 % wbboxes that are worse than the current one
...
%  cout << "TOTAL_EDIT_DISTANCE = " << total_edit_distance << endl;
 fprintf('TOTAL_EDIT_DISTANCE =%s', total_edit_distance); 
