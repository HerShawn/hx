...
dir_img = dir([do_dir 'contour_2015train_detection\*.txt'] );
dir_img2= dir([do_dir 'neumann_2015train-detection\*.txt'] );
num_img = length(dir_img);
for indexImg = 1:num_img
    img_value = dir_img(indexImg).name;
    img_value = img_value(1:end-4); 
    img_value2=dir_img2(indexImg).name;
    img_value2 = img_value2(1:end-4); 
    img_name = [do_dir 'Challenge2_Test_Task12_Images\' img_value '.jpg'];
    g = imread(img_name);
    txt_name = [do_dir 'contour_2015train_detection\' img_value '.txt'];
    txt_name2=[do_dir 'neumann_2015train-detection\' img_value2 '.txt'];
    fid = fopen(txt_name);
...
%     print(indexImg, '-dpng', save_name);
          ...
          target_txt_name = [do_dir 'addContour2Neumann2015\res_img_' num2str(indexImg) '.txt'];
%          target_txt_name=[ 'C:\Users\Administrator\Desktop\1.19日粗定位改进试验5（高宽比；重叠》0.85）\' img_value '.txt'];
         dlmwrite(target_txt_name, dt_nc, 'delimiter', ',','newline', 'pc');
