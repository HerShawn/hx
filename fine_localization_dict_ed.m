% 12/9 DICT&EDIT DISTANCE
function fine_localization_dict_ed(img_gt,gtRes2,g)
...
if size(gtRes2,1)==1
    ...
    predtag = strtrim(lexicon{lexidx})
    ...
    if size(gtRes2,1)>1
        ...
        for i=1:size(gtRes2,1)
            ...
            % for i=1:size(gtRes2,1)
            predwords=predwords_sort(img_gt,i,predwords,bboxes);
        end %看有几个粗定位文本行，然后处理每一个粗定位文本行
        % if size(gtRes2,1)>1
        char_edit_distance(img_gt,predwords);
    end %粗定位检测到多个文本行的情况
    % if size(gtRes2,1)==1
end %粗定位只检测到一个文本行的情况
end