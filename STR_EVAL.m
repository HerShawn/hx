...
%                         states{1}
                    ...
                    if pred_taken(bidx)==0
                        ...
                        for tt = 1:length(img_gt)
                            ...
                            if taken(tt)==0 && pred_taken(bidx)==0 && strcmp(pred_tag, lower(cell2mat(img_gt(tt,5))))
                                pred_y1 =gtRes2(i,2)+aa(1); pred_y2 = gtRes2(i,2)+cc(1);
                                pred_x1 =gtRes2(i,1)+aa(2); pred_x2 =gtRes2(i,1)+bb(2);
                                pred_rec = [pred_x1, pred_y1, pred_x2-pred_x1+1, pred_y2-pred_y1+1];
                                true_rec =[cell2mat(img_gt(tt,1)),cell2mat(img_gt(tt,2)),cell2mat(img_gt(tt,3))-cell2mat(img_gt(tt,1))+1,cell2mat(img_gt(tt,4))-cell2mat(img_gt(tt,2))+1];
                                intersect_area = rectint(pred_rec,true_rec);
                                bounding_width = max(pred_x2, cell2mat(img_gt(tt,1))+cell2mat(img_gt(tt,3))-1)-min(pred_x1,cell2mat(img_gt(tt,1)));
                                bounding_height = max(pred_y2, cell2mat(img_gt(tt,2))+cell2mat(img_gt(tt,4))-1)-min(pred_y1, cell2mat(img_gt(tt,2)));
            ...
            totalTrueBbox = totalTrueBbox+length(img_gt)
        end
