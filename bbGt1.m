...
%   'gtLs','-','dtLs','--','lw',3};
dfs={'evShow',0,'gtShow',1,'dtShow',1,'cols','krg',...
  'gtLs','-','dtLs','--','lw',3};
...
%              hs{k}=bbApply('draw',gt(i,1:4),cols(gt(i,5)+2),lw,gtLs);
         hs{k}=bbApply('draw',gt(i,1:4));
        end;
    end
  if(dtShow), 
      for i=1:size(dt,1), k=k+1;
%        hs{k}=bbApply('draw',dt(i,1:5),cols(dt(i,6)+2),lw,dtLs); 
         hs{k}=bbApply('draw',dt(i,1:4));
      end; 
  end
else
  if(gtShow), k=k+1; hs{k}=bbApply('draw',gt(:,1:4),cols(2),lw,gtLs); end
