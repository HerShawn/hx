...
for indexImg = 3:3
    ...
    if size(gtRes2,1)==1
        ...
        %两者的net是不一样的！！
        predtag = lexicon{lexidx};
        fprintf('gt只有一个时：');
        total_edit_distance = total_edit_distance+EditDistance(lower(cell2mat(img_gt(1,5))),predtag)
        ...
        if size(gtRes2,1)>1
            ...
            for  i2=1: size(img_gt,1)
                ...
                fprintf('gt有多个时 GT word \"%s\" no match found\n', cell2mat(img_gt(i2,5)));
            ...
            for j2=1:length(predwords)
                ...
                if  isempty(find(cell2mat(words_detection_matched(:))==[j2]))
                    ...
                    fprintf('gt有多个时 DT word \"%s\" no match found\n', cell2mat(words_detection_matched(j2)));
...
%                     end
        ...
        if ~isempty(bboxes)
            ...
            for  i2=1: size(img_gt,1)
                %                 cout << " GT word \"" << words_gt[j] << "\" no match found" << endl;
                fprintf('dt有多个时 GT word \"%s\" no match found\n', cell2mat(img_gt(i2,5)));
                total_edit_distance=total_edit_distance+length(cell2mat(img_gt(i2,5)))
            end
            ...
            for j2=1:length(predwords)
                ...
                if  isempty(find(cell2mat(words_detection_matched(:))==[j2]))
                    %                     //cout << " Detection word \"" << words_detection[j] << "\" no match found" << endl;
                    fprintf('dt有多个时 DT word \"%s\" no match found\n', cell2mat(predwords(j2)))
                    total_edit_distance=total_edit_distance+ length(cell2mat(predwords(j2)));
                end
            end
        end
