...
function [newstates, curr]= beam_search(im,states, curr, segs, spacescores, numbeams, thresh, c_split)
...
if currseg == 1 % looking at the 1st seg
    ...
    if scoreTable(1,2)>-90 % score of this candidate path has been computed and stored. No need to compute again.
        ...
        subimg = im(:,segs(1,1):segs(1,2));
        if size(subimg, 3) > 1, subimg = rgb2gray(subimg); end;
        subimg = imresize(subimg, [32, 100]);
        subimg = single(subimg);
        s = std(subimg(:));
        subimg = subimg - mean(subimg(:));
...
%         [score,~] = max(res(end).x(:));
        ...
        [sorted,~]=sort(res(end).x);
        margin=sorted(end,:)-sorted(end-1,:);
        margin=margin(:)';
        %          word=lexicon{lexidx};
        word=s(pred(find(margin>2.5)));
        word_space=s_space(pred(find(margin>2.5)));
        if word==word_space
            score=0;
        else
            score=1;
        end     
         matchscore = score - split_cost;
        wordsTable{1,2} = word;
        ...
        switch states{i}.path(currseg-1)
            ...
            case {1, 3}
                ...
                if scoreTable(currseg,currseg+1)>-90
                    ...
                    subimg = im(:,segs(currseg,1):segs(currseg,2));
                    if size(subimg, 3) > 1, subimg = rgb2gray(subimg); end;
                    subimg = imresize(subimg, [32, 100]);
                    subimg = single(subimg);
                    s = std(subimg(:));
...
%                     [score,~] = max(res(end).x(:));
            ...
            case{2,4}
                ...
                if scoreTable(segbounds(1),segbounds(2)+1)>-90
                    ...
                    subimg = im(:, segs(segbounds(1),1):segs(segbounds(2),2));
                    if size(subimg, 3) > 1, subimg = rgb2gray(subimg); end;
                    subimg = imresize(subimg, [32, 100]);
                    subimg = single(subimg);
                    s = std(subimg(:));
